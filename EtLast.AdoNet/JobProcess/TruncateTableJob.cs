namespace FizzCode.EtLast.AdoNet
{
    ...
    public class TruncateTableJob : AbstractSqlStatementJob
    {
        ...
        {
            process.Context.Log(LogSeverity.Debug, process, "truncating {ConnectionStringKey}/{TableName} with query {SqlStatement}, timeout: {Timeout} sec, transaction: {Transaction}",
            ...
            try
            {
                ...
                process.Context.Log(LogSeverity.Information, process, "{RecordCount} records deleted in {ConnectionStringKey}/{TableName} in {Elapsed}", recordCount, ConnectionStringKey, TableName, startedOn.Elapsed);
            }
            catch (Exception ex)
            {
                ...
                exception.AddOpsMessage(string.Format("database table truncate failed, connection string key: {0}, table: {1}, message: {2}, command: {3}, timeout: {4}",
                    ConnectionStringKey, TableName, ex.Message, originalStatement, CommandTimeout));
