    ...
    public class CopyTableIntoExistingTableProcess : AbstractSqlStatementProcess
    {
        ...
        protected override void RunCommand(IDbCommand command)
        {
            ...
            try
            {
                ...
                Context.CounterCollection.IncrementDebugCounter("db record copy count - " + ConnectionString.Name, recordCount);
                Context.CounterCollection.IncrementDebugCounter("db record copy count - " + ConnectionString.Name + "/" + ConnectionString.Unescape(Configuration.SourceTableName) + " -> " + ConnectionString.Unescape(Configuration.TargetTableName), recordCount);
                Context.CounterCollection.IncrementDebugTimeSpan("db record copy time - " + ConnectionString.Name, time);
                Context.CounterCollection.IncrementDebugTimeSpan("db record copy time - " + ConnectionString.Name + "/" + ConnectionString.Unescape(Configuration.SourceTableName) + " -> " + ConnectionString.Unescape(Configuration.TargetTableName), time);
            }
            catch (Exception ex)
            {
