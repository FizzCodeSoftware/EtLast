            ...
            for (var retry = 0; retry <= MaxRetryCount; retry++)
            {
                ...
                try
                {
                    using (var scope = process.Context.BeginScope(process, null, this, TransactionScopeKind.RequiresNew, LogSeverity.Debug))
                    {
                        ...
                        process.Context.Log(severity, process, null, this, "{TotalRowCount} rows written to {ConnectionStringKey}/{TableName}, micro-transaction: {Transaction}, average speed is {AvgSpeed} msec/Krow), last batch time: {BatchElapsed}",
