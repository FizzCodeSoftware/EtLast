namespace FizzCode.EtLast.Tests
{
    ...
    public class TestExecuterResult
    {
        ...
        /*public string GetGenerateAssertOrderedMatch(params string[] columns)
        {
            ...
            if (MutatedRows.Count > 0)
            {
                sb.AppendLine("Assert.That.OrderedMatch(result, new List<Dictionary<string, string>>() {");
                if (columns?.Length == 0)
                {
                    sb.AppendJoin(",\n", MutatedRows.Select(row => "\t\t\t\tnew Dictionary<string, string>() { " + string.Join(", ", row.Values.Select(kvp => "[\"" + kvp.Key + "\"] = \"" + row.FormatToString(kvp.Key) + "\"")) + " }"));
                }
                else
                {
                    sb.AppendJoin(",\n", MutatedRows.Select(row => "\t\t\t\tnew Dictionary<string, string>() { " + string.Join(", ", columns.Select(col => "[\"" + col + "\"] = " + (row.IsNull(col) ? "null" : ("\"" + row.FormatToString(col) + "\"")))) + " }"));
                }
